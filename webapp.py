...
def result ():
    if request.method == 'POST':
        ...
        datainput = [[hour,age,nmsc,mg,feat, about, accountview, addproperty, addvehicle, adverseactions, alerts, bankverification, 
        ...
        pred = modelFix.predict(datainput)[0]
        proba = modelFix.predict_proba(datainput)[0]
        if pred == 0:
        ...
        return render_template(
            'result.html', hour=hour, age=age, numscreens=nmsc, minigame=strGame, used_premium_feat=strFeat, 
             screens=strScreens, result= rslt, proba = prbb, color = color)


if __name__ == '__main__':
    # coba = joblib.load('model_cobain')
    # gradient = joblib.load('model_gradient')
    modelFix = joblib.load('gradient_model_final')
